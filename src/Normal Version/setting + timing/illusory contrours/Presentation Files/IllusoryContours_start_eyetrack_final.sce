...
bitmap {filename = "white_bkg.bmp"; transparent_color = 128,128,128;} IC_bmp;
bitmap {filename = "white_bkg.bmp"; transparent_color = 128,128,128;} NC_bmp;
...
trial{
	...
	stimulus_event{ nothing{}; deltat = 15;  code = "DotFlip";} dot_evt; # took port_code = 77 so it wouldn't be annoying when testing timing 
} dot_trial;
...
begin
	...
	begin
		...
		if toeyetrack then
			...
			#if (eyedata.x() < initpix) && (eyedata.x() > -1*initpix) && (eyedata.y() < initpix) && (eyedata.y() > -1*initpix) then   #had to command this out, because it wouldn't run without the eyetracker
				inducer_trial.present();
				ISI = random(ISI_interval[1],ISI_interval[2]);
				wait_interval(ISI);
				i = i + 1;
			#end; #had to command this out, because it wouldn't run without the eyetracker
		#else #had to command this out, because it wouldn't run without the eyetracker
		#	inducer_trial.present(); #had to command this out, because it wouldn't run without the eyetracker
		#	ISI = random(ISI_interval[1],ISI_interval[2]); #had to command this out, because it wouldn't run without the eyetracker
		#	wait_interval(ISI); #had to command this out, because it wouldn't run without the eyetracker
		#	i = i + 1; #had to command this out, because it wouldn't run without the eyetracker
		end;
